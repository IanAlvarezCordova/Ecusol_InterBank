...
import {
    LogOut, Search, Wallet, ArrowRightLeft, CheckCircle2, Loader2, Ban, ShieldCheck, Unlock, Power, RotateCcw
} from 'lucide-react';
import { CuentaResumenDTO, MovimientoDTO, ResumenClienteDTO } from '@/types';
...
const DashboardVentanilla = () => {
    ...
    const [tab, setTab] = useState<'DEPOSITO' | 'RETIRO' | 'TRANSFERENCIA' | 'TRANSACCIONES'>('DEPOSITO');
    ...
    const [transacciones, setTransacciones] = useState<MovimientoDTO[]>([]);
    const [loadingTransacciones, setLoadingTransacciones] = useState(false);
    const [procesandoDevolucion, setProcesandoDevolucion] = useState<string | null>(null);
    ...
    const cargarTransacciones = async (numeroCuenta: string) => {
        setLoadingTransacciones(true);
        setTransacciones([]);
        try {
            const data = await ventanillaService.obtenerTransacciones(numeroCuenta);
    ...
    const esDentro48Horas = (fechaString: string): boolean => {
        try {
    ...
    const handleDevolucion = async (instructionId: string | undefined) => {
        if (!instructionId) {
            toast.error('ID de transacción no disponible');
            return;
        }

        if (!confirm('¿Confirma la devolución de esta transacción?')) return;

        setProcesandoDevolucion(instructionId);
        try {
            await ventanillaService.procesarDevolucion(instructionId);
            toast.success('Devolución procesada exitosamente');
            if (cuentaActiva) {
                await cargarTransacciones(cuentaActiva.numeroCuenta);
            }
        } catch (error: any) {
    ...
    const handleOperar = async (e: React.FormEvent) => {
        ...
        // Asegurar que tab sea una operación válida (no TRANSACCIONES)
        if (tab === 'TRANSACCIONES') {
            toast.error("No se puede operar desde la pestaña de transacciones");
            return;
        }
                                    ...
                                    {!cuentaEsActiva ? (
                                        ...
                                        <>
                                            <div className="flex border-b">
                                                {['DEPOSITO', 'RETIRO', 'TRANSFERENCIA', 'TRANSACCIONES'].map((t) => (
                                                    <button
                                                        key={t}
                                                        onClick={() => {
                                                            setTab(t as any);
                                                            if (t === 'TRANSACCIONES') {
                                                                cargarTransacciones(cuentaActiva!.numeroCuenta);
                                                            }
                                                        }}
                                                        className={`flex-1 py-4 font-bold text-sm border-b-4 transition-all ${tab === t ? 'border-nexus-primario text-nexus-primario bg-green-50/30' : 'border-transparent text-gray-400 hover:bg-gray-50'}`}
                                                    >
                                                        {t}
                                                    </button>
                                                ))}
                                                    ...
                                                    <>
                                                        {loadingTransacciones ? (
                                                            <div className="flex justify-center items-center h-40">
                                                                <Loader2 className="animate-spin text-nexus-primario" size={40} />
                                                            </div>
                                                        ) : transacciones.length === 0 ? (
                                                            <div className="text-center py-10 text-gray-500">
                                                                <p className="font-bold">No hay transacciones registradas</p>
                                                            </div>
                                                        ) : (
                                                            <div className="space-y-3">
                                                                {transacciones.map((tx) => {
                                                                    const fechaRaw = tx.fechaEjecucion;
                                                                    let fechaObj: Date;
                                                                    if (Array.isArray(fechaRaw)) {
                                                                        fechaObj = new Date(fechaRaw[0], fechaRaw[1] - 1, fechaRaw[2], fechaRaw[3], fechaRaw[4], fechaRaw[5]);
                                                                    } else {
                                                                        fechaObj = new Date(fechaRaw);
                                                                    }

                                                                    const dentroDelPlazo = esDentro48Horas(fechaRaw ? fechaObj.toISOString() : new Date().toISOString());
                                                                    const fecha = isNaN(fechaObj.getTime()) ? 'Fecha inválida' : fechaObj.toLocaleString('es-ES');

                                                                    let descripcion = tx.descripcion;
                                                                    // Limpiar JSON si existe
                                                                    if (descripcion && descripcion.startsWith("Error: {")) {
                                                                        try {
                                                                            const jsonPart = descripcion.substring(7);
                                                                            // Podemos parsearlo o simplemente mostrar un texto genérico
                                                                            descripcion = "Transacción Fallida (Detalle Técnico Oculto)";
                                                                        } catch (e) { }
                                                                    }

                                                                    return (
                                                                        <div key={tx.instructionId || tx.transaccionId} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow bg-white">
                                                                            <div className="flex justify-between items-start mb-3">
                                                                                <div className="flex-1">
                                                                                    <div className="flex items-center gap-2 mb-1">
                                                                                        <span className="font-bold text-gray-800">{tx.tipo}</span>
                                                                                        <span className={`text-xs px-2 py-1 rounded font-bold ${tx.rolTransaccion === 'CREDITO' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                                                                            {tx.rolTransaccion || (tx.tipo === 'DEPOSITO' ? 'CREDITO' : 'DEBITO')}
                                                                                        </span>
                                                                                        <span className="text-xs bg-gray-100 px-2 py-1 rounded text-gray-600">{tx.estado || 'COMPLETED'}</span>
                                                                                    </div>
                                                                                    <p className="text-sm text-gray-600 font-medium">{descripcion}</p>
                                                                                    <p className="text-xs text-gray-400 mt-1">{fecha}</p>
                                                                                </div>
                                                                                <div className="text-right">
                                                                                    <p className={`font-bold text-lg ${tx.rolTransaccion === 'CREDITO' ? 'text-green-600' : 'text-red-600'}`}>
                                                                                        {tx.rolTransaccion === 'CREDITO' ? '+' : '-'}{formatCurrency(tx.monto)}
                                                                                    </p>
                                                                                </div>
                                                                            </div>
                                                                            ...
                                                                            {tx.cuentaDestino && (
                                                                                <div className="text-xs text-gray-600 mb-3 bg-gray-50 p-2 rounded">
                                                                                    <strong>Destino:</strong> {tx.nombreDestino || tx.cuentaDestino}
                                                                                </div>
                                                                            )}

                                                                            {dentroDelPlazo && (
                                                                                <button
                                                                                    onClick={() => handleDevolucion(tx.instructionId)}
                                                                                    disabled={procesandoDevolucion === tx.instructionId}
                                                                                    className="w-full py-2 mt-2 bg-orange-50 text-orange-700 rounded-lg font-bold text-sm hover:bg-orange-100 disabled:opacity-50 flex items-center justify-center gap-2 transition-colors border border-orange-200"
                                                                                >
                                                                                    {procesandoDevolucion === tx.instructionId ? (
