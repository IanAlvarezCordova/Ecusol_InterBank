                                                ...
                                                {loadingTransacciones ? (
                                                    ...
                                                    <div className="space-y-3">
                                                        {transacciones.map((tx) => {
                                                            const dentroDelPlazo = esDentro48Horas(tx.fechaCreacion);
                                                            const fecha = new Date(tx.fechaCreacion).toLocaleString('es-ES');
                                                            
                                                            return (
                                                                <div key={tx.instructionId || tx.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                                                                    <div className="flex justify-between items-start mb-3">
                                                                        <div className="flex-1">
                                                                            <div className="flex items-center gap-2 mb-1">
                                                                                <span className="font-bold text-gray-800">{tx.tipo}</span>
                                                                                <span className="text-xs bg-gray-100 px-2 py-1 rounded text-gray-600">{tx.estado}</span>
                                                                            </div>
                                                                            <p className="text-sm text-gray-500">{tx.descripcion}</p>
                                                                            <p className="text-xs text-gray-400 mt-1">{fecha}</p>
                                                                        </div>
                                                                        <div className="text-right">
                                                                            <p className="font-bold text-lg text-nexus-primario">{formatCurrency(tx.monto)}</p>
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    {dentroDelPlazo && (
                                                                        <button
                                                                            ...
                                                                            className="w-full py-2 mt-2 bg-orange-100 text-orange-700 rounded-lg font-bold text-sm hover:bg-orange-200 disabled:opacity-50 flex items-center justify-center gap-2 transition-colors"
                                                                        >
