    ...
    <div className="min-h-screen bg-gray-100 font-sans text-gray-800 pb-20">
      <header className="bg-white border-b border-gray-200 px-6 h-16 flex justify-between items-center sticky top-0 z-30 shadow-sm">
         <div className="flex items-center gap-3">
             <div className="bg-ecusol-primario text-white text-xs font-bold px-2 py-1 rounded">ecusol</div>
             <span className="font-bold text-gray-700 hidden sm:inline">Ventanilla: {nombreSucursal}</span>
         </div>
         <div className="flex items-center gap-4">
             <span className="text-sm font-bold text-ecusol-primario uppercase">{usuarioEmpleado}</span>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex gap-4">
            <div className="flex-1 relative">
                ...
                <input 
                    ...
                    className="w-full pl-12 pr-4 py-3 border rounded-lg focus:ring-2 focus:ring-ecusol-primario outline-none" 
                    value={busqueda} 
            ...
            <button onClick={handleBuscar} disabled={loadingBusqueda || !busqueda} className="bg-ecusol-primario text-white px-6 rounded-lg font-bold hover:bg-green-900 disabled:opacity-50 min-w-[100px] flex justify-center items-center">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className={`p-6 rounded-xl shadow-sm border transition-colors ${clienteInactivo ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'}`}>
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="w-12 h-12 bg-ecusol-secundario text-white rounded-full flex items-center justify-center font-bold text-xl">
                             ...
                             return (
                                <div 
                                    ...
                                    className={`p-4 rounded-xl border-2 cursor-pointer transition-all relative overflow-hidden
                                        ${cuentaActiva?.numeroCuenta === cuenta.numeroCuenta ? 'border-ecusol-primario bg-green-50 ring-1 ring-ecusol-primario' : 'border-gray-200 bg-white hover:border-green-300'}
                                ...
                                >
                                    ...
                                    <p className="text-xl font-bold text-ecusol-primario mt-1">{formatCurrency(cuenta.saldo)}</p>
                                </div>
                             );
                        })}
                    </div>
                    ...
                    {clienteInactivo ? (
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden h-full flex flex-col">
                            <div className="bg-gray-50 border-b p-4 flex justify-between items-center">
                                <span className="font-bold text-gray-700 flex items-center gap-2"><Wallet size={18} className="text-ecusol-primario"/> Operando: {cuentaActiva.numeroCuenta}</span>
                            ...
                            {!cuentaEsActiva ? (
                                <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
                                    ...
                                    <button onClick={activarCuenta} disabled={procesando} className="bg-ecusol-primario text-white px-6 py-3 rounded-xl font-bold hover:bg-green-900 flex items-center gap-2 shadow-lg">
                                ...
                                <>
                                    <div className="flex border-b">
                                        {['DEPOSITO', 'RETIRO', 'TRANSFERENCIA'].map((t) => (
                                            <button key={t} onClick={() => setTab(t as any)} className={`flex-1 py-4 font-bold text-sm border-b-4 transition-all ${tab === t ? 'border-ecusol-primario text-ecusol-primario bg-green-50/30' : 'border-transparent text-gray-400 hover:bg-gray-50'}`}>{t}</button>
                                        ))}
                                    </div>
                                    <div className="p-8 flex-1">
                                        <form onSubmit={handleOperar} className="max-w-md mx-auto space-y-6">
                                            ...
                                            <div>
                                                ...
                                                <input type="number" step="0.01" className="w-full pl-4 p-4 text-3xl font-bold border-2 border-gray-200 rounded-xl outline-none focus:border-ecusol-primario" placeholder="0.00" value={monto} onChange={e => setMonto(e.target.value)} required />
                                            </div>
                                            <input type="text" className="w-full p-3 border-2 border-gray-200 rounded-xl outline-none focus:border-ecusol-primario" placeholder="Concepto..." value={descripcion} onChange={e => setDescripcion(e.target.value)} />
                                            <button type="submit" disabled={procesando} className="w-full py-4 bg-ecusol-primario text-white rounded-xl font-bold shadow-lg hover:bg-green-900 transition-all flex justify-center items-center gap-2 disabled:opacity-50">
