...
public class CuentaService {
    ...
    /**
     * Genera número de cuenta con el BIN de ECUSOL (280900).
     * Formato: 280900XXXX (10 dígitos total)
     */
    private String generarNumeroCuenta() {
        String bin = "280900"; // BIN registrado en Switch DIGICONECU

        // Obtener el último número de cuenta para generar el siguiente secuencial
        String ultimoNumero = cuentaRepo.findTopByNumeroCuentaStartingWithOrderByNumeroCuentaDesc(bin)
                .map(Cuenta::getNumeroCuenta)
                .orElse(bin + "0000");

        // Extraer el secuencial (últimos 4 dígitos) e incrementar
        int secuencial = Integer.parseInt(ultimoNumero.substring(6)) + 1;

        // Formatear con ceros a la izquierda (4 dígitos)
        return bin + String.format("%04d", secuencial);
    }
