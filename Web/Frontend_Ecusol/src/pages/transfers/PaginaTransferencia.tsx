...
import { CheckCircle2, Users, Loader2, AlertTriangle, Wallet, ArrowRight, Star, Save, X, ChevronRight, UserCircle, BookUser } from 'lucide-react';
...
const PaginaTransferencia = () => {
  ...
  const confirmarTransferencia = async () => {
    ...
    try {
      let finalDestino = (modo === 'ECUASOL' && tipoInterno === 'PROPIA') ? destinoPropio : destinoManual;
      let bancoFinal = modo === 'ECUASOL' ? 'ECUASOL' : banco;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {opciones.map((c: CuentaDTO) => (
          <button key={c.cuentaId} type="button" onClick={() => onChange(c.numeroCuenta)}
            className={`text-left p-4 rounded-xl border-2 transition-all flex items-center justify-between relative ${valor === c.numeroCuenta ? 'border-ecusol-primario bg-blue-50 ring-1 ring-ecusol-primario' : 'border-gray-200 hover:border-blue-300 bg-white'}`}>
            <div>
    ...
    <div className="max-w-5xl mx-auto pb-20">
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          {destinatarioData.tipoCuenta && (
                            <span className="text-[9px] bg-green-200 px-2 rounded-full font-bold uppercase text-green-800">{destinatarioData.tipoCuenta}</span>
                          )}
                        </div>
                      </div>
                    </div>
                    {!esBeneficiarioGuardado && (
                      <button type="button" onClick={() => { setAliasParaGuardar(destinatarioData.nombreTitular.split(' ')[0]); setShowSaveModal(true); }} className="p-2 bg-white rounded-lg hover:bg-green-50 text-green-600 border border-green-200 transition-colors flex flex-col items-center gap-1 min-w-[60px]">
                        <Star size={20} />
      ...
      {beneficiarios.length > 0 && (
        <div className="animate-slide-up">
          <h3 className="text-xl font-bold text-gray-700 mb-4 flex items-center gap-2 px-2"><BookUser className="text-ecusol-primario" size={22} /> Agenda de Contactos</h3>
          <div className="bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden divide-y divide-gray-100">
            {beneficiarios.map(ben => (
              <button key={ben.id} onClick={() => { setDestinoManual(ben.numeroCuenta); handleValidar(ben.numeroCuenta); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                className={`w-full text-left p-4 transition-all hover:bg-blue-50 flex items-center gap-4 group ${destinoManual === ben.numeroCuenta ? 'bg-blue-50 border-l-4 border-ecusol-primario' : 'border-l-4 border-transparent'}`}>
                <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg shrink-0 shadow-sm ${destinoManual === ben.numeroCuenta ? 'bg-ecusol-primario text-white' : 'bg-gray-100 text-gray-500 group-hover:bg-white group-hover:text-ecusol-primario'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {ben.tipoCuenta && <span className="text-[10px] text-gray-500 bg-gray-100 border border-gray-200 px-2 py-0.5 rounded-full uppercase font-bold">{ben.tipoCuenta}</span>}
                  </div>
                  <p className="text-sm text-gray-500 truncate">{ben.nombreTitular}</p>
                  <p className="text-xs text-gray-400 font-mono mt-0.5 flex items-center gap-1"><Wallet size={10} /> {ben.numeroCuenta}</p>
                </div>
                <ChevronRight size={20} className="text-gray-300 group-hover:text-ecusol-primario group-hover:translate-x-1 transition-all" />
              </button>
            ))}
          </div>
        </div>
              ...
              {txExitosa ? (
                ...
                <>
                  <div className="space-y-4 bg-gray-50 p-5 rounded-xl border border-gray-100">
                    ...
                    <div className="flex justify-between text-sm"><span className="text-gray-500">Para</span><span className="font-bold text-gray-800 text-right">{modo === 'ECUASOL' && tipoInterno === 'PROPIA' ? 'Mi Cuenta Propia' : destinatarioData?.nombreTitular}</span></div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">Banco Destino</span>
                      <span className={`font-bold text-white px-3 py-1 rounded-full text-xs ${modo === 'ECUASOL' ? 'bg-blue-600' : 'bg-orange-500'}`}>{modo === 'ECUASOL' ? 'Nexus' : (BANCOS_TERCEROS.find(b => b.id === banco)?.nombre || banco)}</span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={confirmarGuardarBeneficiario} disabled={guardandoBen || !aliasParaGuardar} className="flex-1 py-2 rounded-lg bg-ecusol-primario text-white font-bold hover:bg-blue-900 flex justify-center items-center gap-2">
                {guardandoBen ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Guardar
              </button>
            </div>
          </div>
        </div>
